...
pub fn open(filename: *const c_char) -> Result<ManagedConnection, ResultCode> {
    ...
    let rc =
        ResultCode::from_i32(crate::capi::open(filename, &mut db as *mut *mut sqlite3)).unwrap();
    if rc == ResultCode::OK {
...
pub fn randomness(blob: &mut [u8]) {
    crate::capi::randomness(blob.len() as c_int, blob.as_mut_ptr() as *mut c_void)
}
...
impl Drop for ManagedConnection {
    fn drop(&mut self) {
        ...
        let rc = crate::capi::close(self.db);
        if rc != 0 {
...
impl VTabRef for *mut *mut vtab {
    fn set<T>(&self, val: Box<T>) {
        unsafe {
            ...
            **self = raw_val.cast::<crate::capi::vtab>();
        }
    }
}
...
impl CursorRef for *mut *mut vtab_cursor {
    fn set<T>(&self, val: Box<T>) {
        unsafe {
            let raw_val = Box::into_raw(val);
            **self = raw_val.cast::<crate::capi::vtab_cursor>();
        }
    }
}
...
pub fn declare_vtab(db: *mut sqlite3, def: &str) -> Result<ResultCode, ResultCode> {
    ...
    let ret = crate::capi::declare_vtab(db, cstring.as_ptr());
    convert_rc(ret)
}

pub fn vtab_config(db: *mut sqlite3, options: i32) -> Result<ResultCode, ResultCode> {
    let rc = crate::capi::vtab_config(db, options);
    convert_rc(rc)
}
